...
class FeaturedQuestions extends React.Component {
	...
	componentDidMount() {
		...
		var self = this;
		axios.get('http://localhost:3001/api/v1/getFeaturedQuestions')
			.then((res) => {
	...
	render() {
  	...
  	return (
  		rows && <Card className={classes.card}>
  			<CardHeader
  				avatar={
						<Avatar aria-label='Featured Questions' className={classes.avatar}>FE</Avatar>
  				}
  				title='Featured Questions'
  				subheader='Questions with bounty'
  			/>
				<Divider/>
  			<CardContent>
					<List>
...
FeaturedQuestions.propTypes = {
	classes: PropTypes.object.isRequired,
};

export default withStyles(styles)(FeaturedQuestions);
